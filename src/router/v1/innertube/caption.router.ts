...
v1InnertubeCaptionRouter.get('/', async (c: Context<AppSchema>) => {
  ...
  const translateLanguage = parsed.data.tl;

  try {
    const info = await c.get('innertubeSvc').getCaption(videoId, language, translateLanguage, { signal: c.get('signal'), requestId });
    return c.json(info);
  } catch (err) {
    ...
    logger.error('Error in /v1/innertube/caption', { err, mapped, videoId, language, requestId });
